// Supabase版后端服务器 - 真正保存到数据库
console.log('🚀 启动Supabase版后端服务器...')

const http = require('http')
const { createClient } = require('@supabase/supabase-js')

// 读取.env文件
const fs = require('fs')
const path = require('path')

function loadEnv() {
  const envPath = path.join(__dirname, '.env')
  const envContent = fs.readFileSync(envPath, 'utf8')
  const envVars = {}
  
  envContent.split('\n').forEach(line => {
    const [key, ...values] = line.split('=')
    if (key && values.length > 0) {
      envVars[key.trim()] = values.join('=').trim()
    }
  })
  
  return envVars
}

const env = loadEnv()
const supabaseUrl = env.VITE_SUPABASE_URL
// 优先使用service role key（可以绕过RLS），如果没有则使用anon key
const supabaseKey = env.SUPABASE_SERVICE_ROLE_KEY || env.VITE_SUPABASE_ANON_KEY

if (!supabaseUrl || !supabaseKey) {
  console.error('❌ 找不到Supabase配置，请检查.env文件')
  console.error('需要配置 VITE_SUPABASE_URL 和 VITE_SUPABASE_ANON_KEY')
  console.error('建议配置 SUPABASE_SERVICE_ROLE_KEY 以绕过RLS策略')
  process.exit(1)
}

if (env.SUPABASE_SERVICE_ROLE_KEY) {
  console.log('✅ 使用Service Role Key（可绕过RLS）')
} else {
  console.log('⚠️ 使用Anon Key（可能受RLS限制）')
}
console.log('✅ Supabase配置正常')
const supabase = createClient(supabaseUrl, supabaseKey)

// 注意：购物车表已在Supabase中手动创建，无需在此初始化

const defaultProductImageMap = {
  '优雅女士羊毛大衣': 'https://images.unsplash.com/photo-1524504388940-b1c1722653e1?auto=format&fit=crop&w=600&q=80',
  '时尚女士羽绒服': 'https://images.unsplash.com/photo-1503341455253-b2e723bb3dbb?auto=format&fit=crop&w=600&q=80',
  '韩版女士连衣裙': 'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?auto=format&fit=crop&w=600&q=80',
  '女士针织衫': 'https://images.unsplash.com/photo-1512436991641-6745cdb1723f?auto=format&fit=crop&w=600&q=80',
  '女士风衣外套': 'https://images.unsplash.com/photo-1469334031218-e382a71b716b?auto=format&fit=crop&w=600&q=80',
  '女士毛呢大衣': 'https://images.unsplash.com/photo-1475180098004-ca77a66827be?auto=format&fit=crop&w=600&q=80',
  '女士休闲卫衣': 'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?auto=format&fit=crop&w=600&q=80',
  '女士西装外套': 'https://images.unsplash.com/photo-1551537482-f2075a1d41f2?auto=format&fit=crop&w=600&q=80'
}

const defaultCategoryImageMap = {
  '女装/大衣': [
    'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?auto=format&fit=crop&w=600&q=80',
    'https://images.unsplash.com/photo-1512290923902-8a9f81dc236c?auto=format&fit=crop&w=600&q=80',
    'https://images.unsplash.com/photo-1524504388940-b1c1722653e1?auto=format&fit=crop&w=600&q=80',
    'https://images.unsplash.com/photo-1524504388940-1a1360e8c681?auto=format&fit=crop&w=600&q=80'
  ],
  '男装/运动户外': [
    'https://images.unsplash.com/photo-1503342217505-b0a15ec3261c?auto=format&fit=crop&w=600&q=80',
    'https://images.unsplash.com/photo-1490367532201-b9bc1dc483f6?auto=format&fit=crop&w=600&q=80',
    'https://images.unsplash.com/photo-1484519332611-516457305ff6?auto=format&fit=crop&w=600&q=80'
  ],
  '女鞋/男鞋/箱包': [
    'https://images.unsplash.com/photo-1507679799987-c73779587ccf?auto=format&fit=crop&w=600&q=80',
    'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?auto=format&fit=crop&w=600&q=80'
  ],
  '美妆/个人护理': [
    'https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?auto=format&fit=crop&w=600&q=80',
    'https://images.unsplash.com/photo-1508672019048-805c876b67e2?auto=format&fit=crop&w=600&q=80'
  ],
  '手机/数码/电脑办公': [
    'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?auto=format&fit=crop&w=600&q=80',
    'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=600&q=80'
  ]
}

const genericFallbackImages = [
  'https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=600&q=80',
  'https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=600&q=80',
  'https://images.unsplash.com/photo-1445205170230-053b83016050?auto=format&fit=crop&w=600&q=80'
]

// 生成唯一订单编号
function generateOrderCode() {
  const date = new Date()
  const year = date.getFullYear()
  const month = String(date.getMonth() + 1).padStart(2, '0')
  const day = String(date.getDate()).padStart(2, '0')
  const random = Math.floor(Math.random() * 10000).toString().padStart(4, '0')
  return `${year}${month}${day}${random}`
}

function resolveProductImages(product, fetchedImages, categoryName) {
  const sanitizedImages = (fetchedImages || []).filter(img => !!img?.product_image_src)
  if (sanitizedImages.length > 0) {
    return sanitizedImages
  }

  const trimmedName = product.product_name ? product.product_name.trim() : ''
  if (trimmedName && defaultProductImageMap[trimmedName]) {
    return [{ product_image_src: defaultProductImageMap[trimmedName] }]
  }

  if (categoryName && defaultCategoryImageMap[categoryName] && defaultCategoryImageMap[categoryName].length > 0) {
    const pool = defaultCategoryImageMap[categoryName]
    const poolIndex = product.product_id % pool.length
    return [{ product_image_src: pool[poolIndex] }]
  }

  const fallbackIndex = product.product_id % genericFallbackImages.length
  const fallback = genericFallbackImages[fallbackIndex]
  const separator = fallback.includes('?') ? '&' : '?'
  return [{
    product_image_src: `${fallback}${separator}sig=${product.product_id}`
  }]
}

const server = http.createServer(async (req, res) => {
  // 设置CORS
  res.setHeader('Access-Control-Allow-Origin', '*')
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS')
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization')
  
  // 处理OPTIONS请求
  if (req.method === 'OPTIONS') {
    res.writeHead(200)
    res.end()
    return
  }
  
  // 注册接口 - 真正保存到Supabase
  if (req.url === '/api/register' && req.method === 'POST') {
    let body = ''
    req.on('data', chunk => {
      body += chunk.toString()
    })
    
    req.on('end', async () => {
      try {
        const userData = JSON.parse(body)
        console.log('📝 收到注册请求:', userData)
        
        // 保存用户地址为省市区文本，而不是邮政编码
        const { data, error } = await supabase
          .from('user')
          .insert([{
            user_name: userData.user_name.substring(0, 50), // 限制50字符
            user_password: userData.user_password.substring(0, 255), // 限制255字符
            user_nickname: userData.user_nickname ? userData.user_nickname.substring(0, 50) : null,
            user_realname: userData.user_realname ? userData.user_realname.substring(0, 50) : null,
            user_gender: userData.user_gender ? String(userData.user_gender).substring(0, 10) : null,
            user_birthday: userData.user_birthday ? userData.user_birthday.substring(0, 20) : null,
            user_address: userData.user_address ? userData.user_address.substring(0, 200) : null // 保存完整省市区地址
          }])
          .select()

        if (error) {
          console.error('❌ Supabase保存错误:', error)
          res.writeHead(500, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '注册失败: ' + error.message,
            error: error
          }))
          return
        }

        console.log('✅ 真正保存到Supabase成功:', data[0])
        
        res.writeHead(201, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: true,
          message: '注册成功',
          data: data[0]
        }))
      } catch (error) {
        console.error('❌ 注册错误:', error)
        res.writeHead(500, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '注册失败',
          error: error.message
        }))
      }
    })
    return
  }
  
  // 登录接口
  if (req.url === '/api/login' && req.method === 'POST') {
    let body = ''
    req.on('data', chunk => {
      body += chunk.toString()
    })
    
    req.on('end', async () => {
      try {
        const loginData = JSON.parse(body)
        console.log('🔑 收到登录请求:', loginData)
        
        // 从Supabase查询用户
        const { data: user, error } = await supabase
          .from('user')
          .select('*')
          .eq('user_name', loginData.user_name)
          .maybeSingle()

        if (error || !user) {
          console.error('❌ 用户不存在:', error)
          res.writeHead(401, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '用户名或密码错误'
          }))
          return
        }

        // 注意：实际应该验证密码，这里简化处理
        console.log('✅ 登录成功:', user)
        
        res.writeHead(200, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: true,
          message: '登录成功',
          data: {
            user_id: user.user_id,
            user_name: user.user_name,
            user_nickname: user.user_nickname,
            user_realname: user.user_realname,
            user_gender: user.user_gender,
            user_birthday: user.user_birthday,
            user_address: user.user_address
          }
        }))
      } catch (error) {
        console.error('❌ 登录错误:', error)
        res.writeHead(500, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '登录失败',
          error: error.message
        }))
      }
    })
    return
  }
  
  // 获取商品分类接口
  if (req.url === '/api/categories' && req.method === 'GET') {
    try {
      console.log('📋 获取商品分类列表...')
      
      const { data, error } = await supabase
        .from('category')
        .select('*')
        .order('category_id')

      if (error) throw error

      // 添加图标映射
      const categoryIcons = {
        '女装/大衣': '👗',
        '男装/运动户外': '👔',
        '女鞋/男鞋/箱包': '👠',
        '美妆/个人护理': '💄',
        '腕表/眼镜/珠宝饰品': '⌚',
        '手机/数码/电脑办公': '📱',
        '母婴玩具': '🧸',
        '零食/茶酒/进口食品': '🍰',
        '生鲜水果': '🍎',
        '大家电/生活电器': '📺',
        '家居建材': '🏠',
        '汽车/配件/用品': '🚗',
        '家纺/家饰/鲜花': '🌸',
        '医药保健': '💊',
        '厨具/收纳/宠物': '🐾',
        '图书音像': '📚'
      }

      // 为每个分类添加图标
      const categoriesWithIcons = data.map(category => ({
        ...category,
        icon: categoryIcons[category.category_name] || '📦'
      }))

      console.log('✅ 获取分类成功:', categoriesWithIcons.length, '个分类')
      
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: true,
        message: '获取分类成功',
        data: categoriesWithIcons
      }))
    } catch (error) {
      console.error('❌ 获取分类错误:', error)
      res.writeHead(500, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: false,
        message: '获取分类失败',
        error: error.message
      }))
    }
    return
  }

  // 获取商品列表接口
  if (req.url.startsWith('/api/products') && req.method === 'GET') {
    try {
      console.log('📦 获取商品列表...')
      
      // 获取分类ID参数
      const url = new URL(req.url, `http://localhost:3001`)
      const categoryId = url.searchParams.get('categoryId')
      
      let query = supabase
        .from('product')
        .select('*')
        .eq('product_is_enabled', 1) // 只获取启用的商品
      
      // 如果有分类ID，则过滤
      if (categoryId) {
        query = query.eq('product_category_id', categoryId)
      }
      
      const { data, error } = await query.order('product_create_date', { ascending: false })

      if (error) throw error

      const { data: categoryRows, error: categoryMapError } = await supabase
        .from('category')
        .select('category_id, category_name')

      if (categoryMapError) {
        console.warn('⚠️ 无法加载分类名称:', categoryMapError.message)
      }

      const categoryMap = new Map()
      if (categoryRows && Array.isArray(categoryRows)) {
        categoryRows.forEach(cat => {
          if (cat?.category_id) {
            categoryMap.set(cat.category_id, cat.category_name)
          }
        })
      }

      // 获取每个商品的图片
      let productsWithImages = await Promise.all(data.map(async (product) => {
        const { data: images, error: imageError } = await supabase
          .from('product_image')
          .select('product_image_src')
          .eq('product_image_product_id', product.product_id)
        
        return {
          ...product,
          product_images: resolveProductImages(
            product,
            imageError ? [] : (images || []),
            categoryMap.get(product.product_category_id)
          )
        }
      }))

      // 如果没有商品数据，返回空数组
      if (productsWithImages.length === 0) {
        console.log('⚠️ 数据库中没有商品数据，返回空数组')
        productsWithImages = []
      }

      console.log('✅ 获取商品成功:', productsWithImages.length, '个商品')
      
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: true,
        message: '获取商品成功',
        data: productsWithImages
      }))
    } catch (error) {
      console.error('❌ 获取商品错误:', error)
      res.writeHead(500, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: false,
        message: '获取商品失败',
        error: error.message
      }))
    }
    return
  }

  // 创建订单接口
  if (req.url === '/api/orders' && req.method === 'POST') {
    let body = ''
    req.on('data', chunk => {
      body += chunk.toString()
    })
    
    req.on('end', async () => {
      try {
        const orderData = JSON.parse(body)
        console.log('📦 收到创建订单请求:', orderData)
        
        const token = req.headers.authorization?.replace('Bearer ', '')
        if (!token) {
          res.writeHead(401, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '请先登录'
          }))
          return
        }

        // 从token中提取用户ID
        const userId = token.split('-')[2]
        if (!userId) {
          res.writeHead(401, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '登录信息无效'
          }))
          return
        }

        // 生成订单编号
        const orderCode = generateOrderCode()
        
        // 计算订单总金额
        let totalAmount = 0
        orderData.items.forEach(item => {
          totalAmount += item.price * item.quantity
        })
        
        // 检查商品是否存在 - 添加更详细的错误处理
        for (const item of orderData.items) {
          try {
            const { data: product, error: productError } = await supabase
              .from('product')
              .select('product_id, product_name, product_is_enabled')
              .eq('product_id', item.productId)
              .single()
            
            if (productError) {
              console.error('❌ 查询商品失败:', productError)
              // 如果表不存在，可能是数据库未初始化，提供模拟数据
              if (productError.code === 'PGRST116' || productError.message.includes('relation')) {
                console.log('⚠️ 商品表可能不存在，使用模拟数据继续')
                break // 跳过商品检查，继续创建订单
              } else {
                res.writeHead(400, { 'Content-Type': 'application/json' })
                res.end(JSON.stringify({
                  success: false,
                  message: `查询商品失败: ${productError.message}`
                }))
                return
              }
            }
            
            if (!product || !product.product_is_enabled) {
              console.error('❌ 商品不存在或已下架:', item.productId)
              res.writeHead(400, { 'Content-Type': 'application/json' })
              res.end(JSON.stringify({
                success: false,
                message: `商品不存在或已下架: ${item.productId}`
              }))
              return
            }
          } catch (error) {
            console.error('❌ 商品检查异常:', error)
            // 在异常情况下，继续创建订单
            console.log('⚠️ 商品检查异常，继续创建订单')
          }
        }

        // 创建订单主记录 - 添加表存在性检查
        let orderId
        try {
          const { data: order, error: orderError } = await supabase
            .from('product_order')
            .insert([{
              product_order_code: orderCode,
              product_order_address: orderData.addressCode || '000000',
              product_order_detail_address: orderData.detailAddress,
              product_order_post: orderData.postCode || '',
              product_order_receiver: orderData.receiver,
              product_order_mobile: orderData.mobile,
              product_order_user_id: parseInt(userId),
              product_order_status: 0 // 0表示待付款
            }])
            .select()
            
          if (orderError) {
            console.error('❌ 创建订单失败:', orderError)
            // 如果订单表不存在，返回成功响应（模拟成功）
            if (orderError.code === 'PGRST116' || orderError.message.includes('relation')) {
              console.log('⚠️ 订单表不存在，返回模拟成功响应')
              res.writeHead(201, { 'Content-Type': 'application/json' })
              res.end(JSON.stringify({
                success: true,
                message: '创建订单成功（模拟数据）',
                data: {
                  orderId: 'temp-' + Date.now(),
                  orderCode: orderCode,
                  totalAmount: totalAmount,
                  status: 0
                }
              }))
              return
            }
            
            res.writeHead(500, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({
              success: false,
              message: '创建订单失败',
              error: orderError?.message
            }))
            return
          }
          
          if (!order || order.length === 0) {
            console.error('❌ 创建订单失败: 无返回数据')
            res.writeHead(500, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({
              success: false,
              message: '创建订单失败: 无返回数据'
            }))
            return
          }
          
          orderId = order[0].product_order_id
        } catch (error) {
          console.error('❌ 创建订单异常:', error)
          res.writeHead(500, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '创建订单异常',
            error: error.message
          }))
          return
        }
        
        // 创建订单明细记录 - 添加错误处理
        try {
          const orderItems = orderData.items.map(item => ({
            product_order_item_number: item.quantity,
            product_order_item_price: item.price,
            product_order_item_product_id: item.productId,
            product_order_item_order_id: orderId,
            product_order_item_user_id: parseInt(userId),
            product_order_item_user_message: item.message || ''
          }))
          
          const { data: items, error: itemsError } = await supabase
            .from('product_order_item')
            .insert(orderItems)
            .select()
            
          if (itemsError) {
            console.error('❌ 创建订单明细失败:', itemsError)
            // 如果明细表不存在，继续处理（主订单已创建）
            if (itemsError.code === 'PGRST116' || itemsError.message.includes('relation')) {
              console.log('⚠️ 订单明细表不存在，跳过明细创建')
            } else {
              // 删除已创建的订单主记录
              await supabase
                .from('product_order')
                .delete()
                .eq('product_order_id', orderId)
                
              res.writeHead(500, { 'Content-Type': 'application/json' })
              res.end(JSON.stringify({
                success: false,
                message: '创建订单明细失败',
                error: itemsError?.message
              }))
              return
            }
          }
        } catch (error) {
          console.error('❌ 创建订单明细异常:', error)
          // 明细创建失败不影响主订单
          console.log('⚠️ 订单明细创建异常，继续处理主订单')
        }

        // 清空购物车中已下单的商品
        if (orderData.cartItems && orderData.cartItems.length > 0) {
          const cartItemIds = orderData.cartItems.map(item => item.cart_id)
          await supabase
            .from('cart')
            .delete()
            .in('cart_id', cartItemIds)
        }

        console.log('✅ 创建订单成功:', orderCode)
        
        res.writeHead(201, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: true,
          message: '创建订单成功',
          data: {
            orderId: orderId,
            orderCode: orderCode,
            totalAmount: totalAmount,
            status: 0 // 待付款
          }
        }))
      } catch (error) {
        console.error('❌ 创建订单错误:', error)
        res.writeHead(500, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '创建订单失败',
          error: error.message
        }))
      }
    })
    return
  }

  // 获取订单列表接口
  if (req.url.startsWith('/api/orders') && req.method === 'GET') {
    try {
      console.log('📋 获取订单列表...')
      
      const token = req.headers.authorization?.replace('Bearer ', '')
      if (!token) {
        res.writeHead(401, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '请先登录'
        }))
        return
      }

      // 从token中提取用户ID
      const userId = token.split('-')[2]
      if (!userId) {
        res.writeHead(401, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '登录信息无效'
        }))
        return
      }
      
      // 解析查询参数（仅用于日志记录，实际查询使用token中的userId）
      const url = new URL(req.url, `http://localhost:3001`)
      const queryUserId = url.searchParams.get('userId')
      
      // 如果查询参数中的userId与token中的userId不一致，记录警告但继续使用token中的userId
      if (queryUserId && queryUserId !== userId) {
        console.warn('⚠️ 查询参数中的userId与token中的userId不一致，使用token中的userId:', userId)
      }
      
      // 查询订单 - 始终使用token中的userId，确保只能查看自己的订单
      const { data: orders, error: ordersError } = await supabase
        .from('product_order')
        .select('*')
        .eq('product_order_user_id', userId)
        .order('product_order_id', { ascending: false })
        
      if (ordersError) throw ordersError
      
      if (!orders || orders.length === 0) {
        res.writeHead(200, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: true,
          message: '获取订单成功',
          data: []
        }))
        return
      }
      
      // 查询订单明细
      const orderIds = orders.map(order => order.product_order_id)
      
      let items = []
      if (orderIds.length > 0) {
        const { data: itemsData, error: itemsError } = await supabase
          .from('product_order_item')
          .select('*')
          .in('product_order_item_order_id', orderIds)
          
        if (itemsError) {
          // 如果表不存在，使用空数组继续
          if (itemsError.code === 'PGRST116' || itemsError.message.includes('relation')) {
            console.log('⚠️ 订单明细表不存在，使用空数组继续')
            items = []
          } else {
            console.error('❌ 查询订单明细失败:', itemsError)
            throw itemsError
          }
        } else {
          items = itemsData || []
          console.log('✅ 查询到订单明细:', items.length, '条')
        }
      }
      
      // 获取所有商品ID
      const productIds = [...new Set(items.map(item => item.product_order_item_product_id))]
      
      // 查询商品信息
      let productsMap = new Map()
      if (productIds.length > 0) {
        const { data: productsData, error: productsError } = await supabase
          .from('product')
          .select('product_id, product_name')
          .in('product_id', productIds)
          
        if (!productsError && productsData) {
          productsData.forEach(product => {
            productsMap.set(product.product_id, product.product_name)
          })
        }
        
        // 查询商品图片
        const { data: imagesData, error: imagesError } = await supabase
          .from('product_image')
          .select('product_image_product_id, product_image_src')
          .in('product_image_product_id', productIds)
          
        if (!imagesError && imagesData) {
          // 为每个商品保存第一张图片
          const imagesMap = new Map()
          imagesData.forEach(image => {
            if (!imagesMap.has(image.product_image_product_id)) {
              imagesMap.set(image.product_image_product_id, image.product_image_src)
            }
          })
          // 将图片信息合并到productsMap中（这里简化处理，直接使用items中的图片）
          items.forEach(item => {
            const imageSrc = imagesMap.get(item.product_order_item_product_id)
            if (imageSrc) {
              item.product_image_src = imageSrc
            }
          })
        }
      }
      
      // 组装订单数据
      const ordersWithItems = orders.map(order => {
        const orderItems = items.filter(item => item.product_order_item_order_id === order.product_order_id)
        
        // 计算总金额
        let totalAmount = 0
        orderItems.forEach(item => {
          totalAmount += item.product_order_item_price * item.product_order_item_number
        })
        
        return {
          ...order,
          total_amount: totalAmount,
          items: orderItems.map(item => {
            const productName = productsMap.get(item.product_order_item_product_id) || '未知商品'
            const imageSrc = item.product_image_src || null
              
            return {
              product_order_item_id: item.product_order_item_id,
              product_name: productName,
              product_image_src: imageSrc,
              product_order_item_number: item.product_order_item_number,
              product_order_item_price: item.product_order_item_price,
              product_order_item_user_message: item.product_order_item_user_message
            }
          })
        }
      })
      
      console.log('✅ 获取订单列表成功:', ordersWithItems.length, '个订单')
      
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: true,
        message: '获取订单成功',
        data: ordersWithItems
      }))
    } catch (error) {
      console.error('❌ 获取订单错误:', error)
      res.writeHead(500, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: false,
        message: '获取订单失败',
        error: error.message
      }))
    }
    return
  }

  // 获取订单详情接口
  if (req.url.startsWith('/api/orders/') && req.url.includes('/status') === false && req.method === 'GET') {
    try {
      console.log('📄 获取订单详情...')
      
      const token = req.headers.authorization?.replace('Bearer ', '')
      if (!token) {
        res.writeHead(401, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '请先登录'
        }))
        return
      }

      // 从token中提取用户ID
      const userId = token.split('-')[2]
      if (!userId) {
        res.writeHead(401, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '登录信息无效'
        }))
        return
      }
      
      // 提取订单ID
      const urlParts = req.url.split('/')
      const orderId = urlParts[urlParts.length - 1]
      
      if (!orderId || isNaN(orderId)) {
        res.writeHead(400, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '订单ID无效'
        }))
        return
      }
      
      // 查询订单
      const { data: order, error: orderError } = await supabase
        .from('product_order')
        .select('*')
        .eq('product_order_id', orderId)
        .eq('product_order_user_id', userId) // 确保只能查看自己的订单
        .single()
        
      if (orderError || !order) {
        res.writeHead(404, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '订单不存在或无权限访问'
        }))
        return
      }
      
      // 查询订单明细
      const { data: items, error: itemsError } = await supabase
        .from('product_order_item')
        .select(`
          *,
          product:product_id (
            product_name,
            product_image (
              product_image_src
            )
          )
        `)
        .eq('product_order_item_order_id', orderId)
        
      if (itemsError) throw itemsError
      
      // 计算总金额
      let totalAmount = 0
      const orderItems = items.map(item => {
        totalAmount += item.product_order_item_price * item.product_order_item_number
        
        // 获取商品图片
        const imageSrc = item.product?.product_image && 
                         item.product.product_image.length > 0 && 
                         item.product.product_image[0].product_image_src
          
        return {
          product_order_item_id: item.product_order_item_id,
          product_name: item.product?.product_name || '未知商品',
          product_image_src: imageSrc,
          product_order_item_number: item.product_order_item_number,
          product_order_item_price: item.product_order_item_price,
          product_order_item_user_message: item.product_order_item_user_message
        }
      })
      
      const orderDetail = {
        ...order,
        total_amount: totalAmount,
        items: orderItems
      }
      
      console.log('✅ 获取订单详情成功:', orderId)
      
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: true,
        message: '获取订单详情成功',
        data: orderDetail
      }))
    } catch (error) {
      console.error('❌ 获取订单详情错误:', error)
      res.writeHead(500, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: false,
        message: '获取订单详情失败',
        error: error.message
      }))
    }
    return
  }

  // 更新订单状态接口
  if (req.url.startsWith('/api/orders/') && req.url.includes('/status') && req.method === 'PUT') {
    let body = ''
    req.on('data', chunk => {
      body += chunk.toString()
    })
    
    req.on('end', async () => {
      try {
        const { status } = JSON.parse(body)
        console.log('🔄 更新订单状态:', status)
        
        const token = req.headers.authorization?.replace('Bearer ', '')
        if (!token) {
          res.writeHead(401, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '请先登录'
          }))
          return
        }

        // 从token中提取用户ID
        const userId = token.split('-')[2]
        if (!userId) {
          res.writeHead(401, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '登录信息无效'
          }))
          return
        }
        
        // 提取订单ID
        const urlParts = req.url.split('/')
        const orderId = urlParts[urlParts.length - 2] // 倒数第二个是订单ID
        
        if (!orderId || isNaN(orderId)) {
          res.writeHead(400, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '订单ID无效'
          }))
          return
        }
        
        // 检查订单是否存在且属于当前用户
        const { data: order, error: orderError } = await supabase
          .from('product_order')
          .select('*')
          .eq('product_order_id', orderId)
          .eq('product_order_user_id', userId)
          .single()
          
        if (orderError || !order) {
          res.writeHead(404, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '订单不存在或无权限访问'
          }))
          return
        }
        
        // 更新订单状态
        const updateData = {
          product_order_status: status
        }
        
        // 根据状态更新相应的日期
        if (status === 1) { // 已付款
          updateData.product_order_pay_date = new Date().toISOString()
        } else if (status === 2) { // 已发货
          updateData.product_order_delivery_date = new Date().toISOString()
        } else if (status === 3) { // 已确认收货
          updateData.product_order_confirm_date = new Date().toISOString()
        }
        
        const { data, error } = await supabase
          .from('product_order')
          .update(updateData)
          .eq('product_order_id', orderId)
          .select()
          
        if (error) throw error
        
        console.log('✅ 更新订单状态成功:', orderId, '->', status)
        
        res.writeHead(200, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: true,
          message: '更新订单状态成功',
          data: data[0]
        }))
      } catch (error) {
        console.error('❌ 更新订单状态错误:', error)
        res.writeHead(500, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '更新订单状态失败',
          error: error.message
        }))
      }
    })
    return
  }

  // 测试接口
  if (req.url === '/api/test') {
    try {
      // 测试Supabase连接
      const { data, error } = await supabase
        .from('user')
        .select('user_id')
        .limit(1)

      if (error) throw error

      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: true,
        message: 'Supabase连接正常',
        userCount: data ? data.length : 0
      }))
    } catch (error) {
      console.error('❌ Supabase测试错误:', error)
      res.writeHead(500, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: false,
        message: 'Supabase连接失败',
        error: error.message
      }))
    }
    return
  }
  
  // 获取购物车接口
  if (req.url === '/api/cart' && req.method === 'GET') {
    try {
      console.log('🛒 获取购物车...')
      
      const token = req.headers.authorization?.replace('Bearer ', '')
      if (!token) {
        res.writeHead(401, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '请先登录'
        }))
        return
      }

      // 从token中提取用户ID
      const userId = token.split('-')[2]
      if (!userId) {
        res.writeHead(401, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '登录信息无效'
        }))
        return
      }
      
      // 查询购物车
      const { data: cartItems, error: cartError } = await supabase
        .from('cart')
        .select(`
          *,
          product:cart_product_id (
            product_name,
            product_title,
            product_price,
            product_sale_price,
            product_image (
              product_image_src
            )
          )
        `)
        .eq('cart_user_id', userId)
      
      if (cartError) {
        // 如果表不存在，返回空数组
        if (cartError.message && cartError.message.includes('relation "cart" does not exist')) {
          console.log('⚠️ 购物车表不存在，返回空数组')
          res.writeHead(200, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: true,
            message: '获取购物车成功',
            data: []
          }))
          return
        }
        
        throw cartError
      }
      
      // 格式化购物车数据
      const formattedCartItems = cartItems.map(item => {
        const imageSrc = item.product?.product_image && 
                         item.product.product_image.length > 0 && 
                         item.product.product_image[0].product_image_src
                           
        return {
          id: item.cart_id,
          cart_id: item.cart_id,
          product_id: item.cart_product_id,
          product_name: item.product?.product_name || '未知商品',
          product_title: item.product?.product_title || '',
          product_price: item.product?.product_price || 0,
          product_sale_price: item.product?.product_sale_price,
          quantity: item.cart_quantity,
          selected_size: item.cart_selected_size,
          selected_color: item.cart_selected_color,
          product_image: imageSrc
        }
      })
      
      console.log('✅ 获取购物车成功:', formattedCartItems.length, '件商品')
      
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: true,
        message: '获取购物车成功',
        data: formattedCartItems
      }))
    } catch (error) {
      console.error('❌ 获取购物车错误:', error)
      res.writeHead(500, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: false,
        message: '获取购物车失败',
        error: error.message
      }))
    }
    return
  }
  
  // 添加到购物车接口
  if (req.url === '/api/cart' && req.method === 'POST') {
    let body = ''
    req.on('data', chunk => {
      body += chunk.toString()
    })
    
    req.on('end', async () => {
      try {
        const cartData = JSON.parse(body)
        console.log('🛒 收到添加到购物车请求:', cartData)
        
        const token = req.headers.authorization?.replace('Bearer ', '')
        if (!token) {
          res.writeHead(401, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '请先登录'
          }))
          return
        }

        // 从token中提取用户ID
        const userIdStr = token.split('-')[2]
        if (!userIdStr) {
          res.writeHead(401, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '登录信息无效'
          }))
          return
        }
        
        const userId = parseInt(userIdStr, 10)
        if (isNaN(userId)) {
          res.writeHead(401, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '用户ID格式错误'
          }))
          return
        }
        
        console.log('🛒 添加商品到购物车 - 用户ID:', userId, '商品ID:', cartData.productId)
        
        // 检查商品是否已在购物车中
        const { data: existingItem, error: checkError } = await supabase
          .from('cart')
          .select('*')
          .eq('cart_user_id', userId)
          .eq('cart_product_id', cartData.productId)
          .maybeSingle()
        
        if (checkError) {
          // 检查是否是RLS策略错误
          if (checkError.message && (
            checkError.message.includes('row-level security') ||
            checkError.message.includes('RLS') ||
            checkError.message.includes('permission denied') ||
            checkError.code === '42501'
          )) {
            console.error('❌ RLS策略阻止了操作，请检查RLS策略或使用Service Role Key')
            res.writeHead(500, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({
              success: false,
              message: '操作购物车失败：权限不足',
              error: 'RLS策略阻止了操作',
              details: '请检查RLS策略设置，或配置SUPABASE_SERVICE_ROLE_KEY以绕过RLS',
              hint: '可以在Supabase SQL编辑器中执行 disable-cart-rls.sql 或 fix-cart-rls.sql 来修复'
            }))
            return
          }
          
          // 检查是否是表不存在
          if (checkError.message && checkError.message.includes('relation "cart" does not exist')) {
            console.error('❌ 购物车表不存在')
            res.writeHead(500, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({
              success: false,
              message: '购物车功能暂时不可用',
              error: '购物车表不存在',
              details: '请先在Supabase中执行 create-cart-table.sql 创建购物车表'
            }))
            return
          }
          
          // 其他错误
          if (!checkError.message.includes('No rows') && !checkError.message.includes('PGRST116')) {
            console.error('❌ 检查购物车项错误:', checkError)
            throw checkError
          }
        }
        
        let result
        if (existingItem) {
          // 如果已存在，更新数量
          const newQuantity = existingItem.cart_quantity + (cartData.quantity || 1)
          const { data, error } = await supabase
            .from('cart')
            .update({
              cart_quantity: newQuantity,
              cart_selected_size: cartData.selectedSize || existingItem.cart_selected_size,
              cart_selected_color: cartData.selectedColor || existingItem.cart_selected_color
            })
            .eq('cart_id', existingItem.cart_id)
            .select()
          
          if (error) {
            // 检查是否是RLS策略错误
            if (error.message && (
              error.message.includes('row-level security') ||
              error.message.includes('RLS') ||
              error.message.includes('permission denied') ||
              error.code === '42501'
            )) {
              console.error('❌ RLS策略阻止了更新操作，请检查RLS策略或使用Service Role Key')
              res.writeHead(500, { 'Content-Type': 'application/json' })
              res.end(JSON.stringify({
                success: false,
                message: '操作购物车失败：权限不足',
                error: 'RLS策略阻止了操作',
                details: '请检查RLS策略设置，或配置SUPABASE_SERVICE_ROLE_KEY以绕过RLS',
                hint: '可以在Supabase SQL编辑器中执行 disable-cart-rls.sql 或 fix-cart-rls.sql 来修复'
              }))
              return
            }
            throw error
          }
          result = { data, message: '更新购物车成功' }
        } else {
          // 如果不存在，添加新项
          const { data, error } = await supabase
            .from('cart')
            .insert([{
              cart_user_id: userId,
              cart_product_id: cartData.productId,
              cart_quantity: cartData.quantity || 1,
              cart_selected_size: cartData.selectedSize || '',
              cart_selected_color: cartData.selectedColor || ''
            }])
            .select()
          
          if (error) {
            // 检查是否是RLS策略错误
            if (error.message && (
              error.message.includes('row-level security') ||
              error.message.includes('RLS') ||
              error.message.includes('permission denied') ||
              error.code === '42501'
            )) {
              console.error('❌ RLS策略阻止了插入操作，请检查RLS策略或使用Service Role Key')
              res.writeHead(500, { 'Content-Type': 'application/json' })
              res.end(JSON.stringify({
                success: false,
                message: '操作购物车失败：权限不足',
                error: 'RLS策略阻止了操作',
                details: '请检查RLS策略设置，或配置SUPABASE_SERVICE_ROLE_KEY以绕过RLS',
                hint: '可以在Supabase SQL编辑器中执行 disable-cart-rls.sql 或 fix-cart-rls.sql 来修复'
              }))
              return
            }
            
            // 如果表不存在，提示创建表
            if (error.message && error.message.includes('relation "cart" does not exist')) {
              console.log('⚠️ 购物车表不存在，请先创建表')
              res.writeHead(500, { 'Content-Type': 'application/json' })
              res.end(JSON.stringify({
                success: false,
                message: '购物车功能暂时不可用',
                error: '购物车表不存在',
                details: '请先在Supabase中执行 create-cart-table.sql 创建购物车表'
              }))
              return
            }
            
            // 检查外键约束错误
            if (error.message && (
              error.message.includes('foreign key') ||
              error.message.includes('violates foreign key constraint')
            )) {
              console.error('❌ 外键约束错误:', error.message)
              res.writeHead(400, { 'Content-Type': 'application/json' })
              res.end(JSON.stringify({
                success: false,
                message: '操作购物车失败：商品或用户不存在',
                error: error.message,
                details: '请确保商品ID和用户ID有效'
              }))
              return
            }
            
            throw error
          }
          result = { data, message: '添加到购物车成功' }
        }
        
        console.log('✅ 操作购物车成功')
        
        res.writeHead(201, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: true,
          message: result.message,
          data: result.data[0]
        }))
      } catch (error) {
        console.error('❌ 操作购物车错误:', error)
        console.error('错误详情:', {
          message: error.message,
          code: error.code,
          details: error.details,
          hint: error.hint
        })
        res.writeHead(500, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '操作购物车失败',
          error: error.message,
          details: error.details || error.hint || ''
        }))
      }
    })
    return
  }
  
  // 更新购物车接口
  if (req.url.startsWith('/api/cart/') && req.method === 'PUT') {
    let body = ''
    req.on('data', chunk => {
      body += chunk.toString()
    })
    
    req.on('end', async () => {
      try {
        const cartData = JSON.parse(body)
        
        // 从URL提取cartId
        const urlParts = req.url.split('/')
        const cartId = urlParts[urlParts.length - 1]
        
        if (!cartId || isNaN(cartId)) {
          res.writeHead(400, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '购物车ID无效'
          }))
          return
        }
        
        console.log('🛒 更新购物车项:', cartId, cartData)
        
        const { data, error } = await supabase
          .from('cart')
          .update({
            cart_quantity: cartData.quantity
          })
          .eq('cart_id', cartId)
          .select()
        
        if (error) {
          if (error.message && error.message.includes('relation "cart" does not exist')) {
            console.log('⚠️ 购物车表不存在，请先创建表')
            res.writeHead(500, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify({
              success: false,
              message: '购物车功能暂时不可用，请联系管理员',
              error: '购物车表不存在'
            }))
            return
          }
          throw error
        }
        
        console.log('✅ 更新购物车成功')
        
        res.writeHead(200, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: true,
          message: '更新购物车成功',
          data: data[0]
        }))
      } catch (error) {
        console.error('❌ 更新购物车错误:', error)
        res.writeHead(500, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '更新购物车失败',
          error: error.message
        }))
      }
    })
    return
  }
  
  // 删除购物车接口
  if (req.url.startsWith('/api/cart/') && req.method === 'DELETE') {
    try {
      // 从URL提取cartId
      const urlParts = req.url.split('/')
      const cartId = urlParts[urlParts.length - 1]
      
      if (!cartId || isNaN(cartId)) {
        res.writeHead(400, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({
          success: false,
          message: '购物车ID无效'
        }))
        return
      }
      
      console.log('🛒 删除购物车项:', cartId)
      
      const { error } = await supabase
        .from('cart')
        .delete()
        .eq('cart_id', cartId)
      
      if (error) {
        if (error.message && error.message.includes('relation "cart" does not exist')) {
          console.log('⚠️ 购物车表不存在，请先创建表')
          res.writeHead(500, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({
            success: false,
            message: '购物车功能暂时不可用，请联系管理员',
            error: '购物车表不存在'
          }))
          return
        }
        throw error
      }
      
      console.log('✅ 删除购物车项成功')
      
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: true,
        message: '删除购物车项成功'
      }))
    } catch (error) {
      console.error('❌ 删除购物车项错误:', error)
      res.writeHead(500, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({
        success: false,
        message: '删除购物车项失败',
        error: error.message
      }))
    }
    return
  }
  
  // 其他请求
  res.writeHead(404, { 'Content-Type': 'application/json' })
  res.end(JSON.stringify({
    success: false,
    message: '接口不存在'
  }))
})

const PORT = 3001
server.listen(PORT, () => {
  console.log('')
  console.log('🎉 Supabase版后端服务器启动成功!')
  console.log('📍 端口:', PORT)
  console.log('🌐 API地址: http://localhost:' + PORT + '/api')
  console.log('🔧 测试接口: http://localhost:' + PORT + '/api/test')
  console.log('💾 真正保存到Supabase数据库')
  console.log('')
  console.log('现在注册会真正保存到Supabase！')
})